...
# @model_validator(mode=’after’)
# декоратор который запускает метод автоматически,
# надо для того чтобы проверять зависимые от друг-друга поля
# mode='after' - значит что проверит сразу после конструктора
# mode='before' - значит что проверит до конструктора а потом вызовет
# конструктор
